...
### 1. Baostock 连接 (已优化)
- **每个线程独立登录 Baostock**，避免共享连接导致的线程安全问题
- 每个线程在开始时登录，结束时自动登出
- **随机启动延迟**: 每个线程启动时随机延迟 0-2 秒，避免同时登录
- **请求间隔**: 每次 Baostock 查询后延迟 0.1 秒
- **自动重试机制**: 网络错误时最多重试 3 次，每次间隔 1 秒
- 有效降低了多线程环境下的网络接收错误 (如 `'utf-8' codec can't decode byte` 错误)
